...
if ($.isNode()) {
	...
	// ].filter((item) => !!item);
	cookiesArr = [$.getdata("CookieJD")].filter((item) => !!item);
}
!(async () => {
	if (!cookiesArr[0]) {
		$.msg(
...
async function showMsg() {
	...
	allMessage += `账号${$.index}：${$.nickName ||
		...
		$.beanCount
	}(今日将过期${$.expirejingdou})京豆 🐶${$.message}${
				...
				if (err) {
					...
					if (data) {
						...
						$.message += `\n当前总红包：${$.balance}(今日总过期${(
							...
							$.jxRed
						}(今日将过期${$.jxRedExpire.toFixed(2)})元 🧧\n极速版红包：${
							$.jsRed
						}(今日将过期${$.jsRedExpire.toFixed(2)})元 🧧\n京东红包：${
							$.jdRed
						}(今日将过期${$.jdRedExpire.toFixed(2)})元 🧧\n健康红包：${
							$.jdhRed
						}(今日将过期${$.jdhRedExpire.toFixed(2)})元 🧧`;
