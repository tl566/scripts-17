...
async function modifyRemark() {
	...
	for (let i = 0; i < envs.length; i++) {
		if (envs[i].value) {
			var tempid = 0;
			tempptpin = decodeURIComponent(
				envs[i].value.match(/pt_pin=([^; ]+)(?=;?)/) &&
					envs[i].value.match(/pt_pin=([^; ]+)(?=;?)/)[1]
			);
			if (envs[i]._id) tempid = envs[i]._id;
			if (envs[i].id) tempid = envs[i].id;
			...
			if (struid) {
				...
				if (Tempindex != -1) {
					...
					strRemark =
						strRemark + "@@" + DateTimestamp.getTime() + "@@" + struid;
				}
				console.log(strRemark);
				if (envs[i]._id) {
					var updateEnvBody = await updateEnv(cookie, envs[i]._id, strRemark);

					if (updateEnvBody.code == 200) console.log("更新Remark成功!");
					else console.log("更新Remark失败!");
				}
				if (envs[i].id) {
					var updateEnvBody = await updateEnv11(cookie, envs[i].id, strRemark);

					if (updateEnvBody.code == 200) console.log("新版青龙更新Remark成功!");
					else console.log("新版青龙更新Remark失败!");
				}
			}
		}
	}
